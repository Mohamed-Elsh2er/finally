    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 p-6 flex items-center justify-center">
        <div className="text-center">
          ...
          <h3 className="text-xl font-semibold text-gray-600 mb-2">Error Loading Categories</h3>
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-gray-800 mb-4">Shop by Categories</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 mb-12">
          {categories.map((category) => (
            <div
              key={category._id}
              onClick={() => getCategoryProducts(category._id)}
              className={`category-card cursor-pointer group relative overflow-hidden rounded-xl shadow-lg border transition-all duration-300 transform hover:scale-105 ${
                selectedCategory === category._id ? 'ring-4 ring-green-500' : ''
              }`}
            >
              <div className="aspect-square bg-[var(--card-bg)] flex items-center justify-center">
                <div className="text-center p-4">
                  <div className="w-16 h-16 mx-auto mb-3 rounded-full flex items-center justify-center bg-gray-100">
                    <img
                      src={category.image || 'https://via.placeholder.com/100x100?text=Category'}
                      alt={category.name}
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            ...
            <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
              {categoryProducts.map((product) => (
                <div
                  key={product._id}
                  className="cursor-pointer group overflow-hidden shadow-lg p-4 pb-16 relative transition-transform duration-300 border border-transparent hover:border-green-500 rounded-xl hover:scale-105 bg-white"
                >
                    ...
                    <div className="flex justify-between items-center mb-2">
                      {product.priceAfterDiscount ? (
                        <>
                          ...
                          <p className="font-bold text-green-600">{product.priceAfterDiscount} EGP</p>
                        </>
                      ) : (
                  ...
                  <div className="flex gap-2 absolute bottom-4 left-1/2 -translate-x-1/2">
                    <button 
                      ...
                      className="group-hover:translate-y-0 translate-y-[200%] group-hover:opacity-100 opacity-0 transition duration-500 ease-out bg-green-500 hover:bg-green-600 text-white font-semibold px-3 py-2 rounded-lg shadow-lg text-sm"
                    >
                      Add to Cart
                    </button>
                    <button 
